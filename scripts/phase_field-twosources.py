...
def run_CH(args):
    ...
    **Input variables**
    ...
    for par in par_values:
        ...
        input_parameters[par_name] = par; # culprit
        ...
        k_production_list = input_parameters['k_production']
        k_degradation = input_parameters['k_degradation'];
        k_production = np.array(k_production_list).sum()
        ...
        if 'multiplier' in input_parameters.keys():
            rates_1 = f_en.RNA_reactions(k_production=k_production_list,
...
#                                        scale=localization_scale);
        ...
        phi_p[:] =fp.GaussianNoiseVariable(mesh=mesh,mean=phi_p_0,variance=0.0*phi_p_0).value
